...
def calculate_measures(work_name, model_tag, val_tag, measures, save=False, beta=np.sqrt(0.3), gt_threshold=0.5):
    """
    ...
    # 预测图像路径
    sm_dir = os.path.join('/data2021/tb/AllWork', work_name, 'Detect', model_tag, val_tag)
    # 真实标签路径
    gt_dir = os.path.join('/data2021/tb', val_tag, 'val/Masks')
    ...
    setting_file_path = os.path.join('/data2021/tb/CodeTable', 'settings_' + work_name[-1] + '.json')
    ...
    if 'MAE' in measures:
        ...
        values['MAE'] = setting[model_tag][val_tag]['MAE']

    if 'E-measure' in measures:
        values['E-measure'] = np.mean(values['E-measure'])
        values['E-measure'] = setting[model_tag][val_tag]['Em']

    if 'S-measure' in measures:
        values['S-measure'] = np.mean(values['S-measure'])
        values['S-measure'] = setting[model_tag][val_tag]['Sm']

    if 'Adp-F' in measures:
        values['Adp-F'] = np.mean(values['Adp-F'])
        values['Adp-F'] = setting[model_tag][val_tag]['Af']

    if 'Wgt-F' in measures:
        values['Wgt-F'] = np.mean(values['Wgt-F'])
        values['Wgt-F'] = setting[model_tag][val_tag]['Wf']
    ...
    if 'Max-F' in measures:
        ...
        values['Max-F'] = setting[model_tag][val_tag]['Fm']
...
def centroid(gt):
    ...
    if np.sum(gt) == 0:
        ...
        x = np.round(np.sum(np.sum(gt, 0, keepdims=True) * i) / total)
        y = np.round(np.sum(np.sum(gt, 1, keepdims=True) * j) / total)
...
def object(gt, sm):
    # if pred is completely black
    right_pixel = np.sum(sm[gt == 1])
    if right_pixel == 0.0:
        return 0
