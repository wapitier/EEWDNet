...
class SOD_Dataset(data.Dataset):
    def __init__(self, train: bool = True, transform=None):
        ...
        train_set_2 = '/data2021/tb/HRSOD_1024'

        val_set = '/data2021/tb/DUTS_1024'

        if train:
            tr1_path = glob.glob(os.path.join(train_set_1, 'train', 'Images', '*.jpg'))
            tr2_path = glob.glob(os.path.join(train_set_2, 'train', 'Images', '*.jpg'))
            all_img_path = tr1_path + tr2_path
...
#     train_dataset = DUTSDataset(train=True)
    ...
    # val_dataset = DUTSDataset(rootdir, train=False)
